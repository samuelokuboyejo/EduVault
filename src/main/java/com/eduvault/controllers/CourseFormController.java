...
import com.eduvault.services.CourseFormService;
...
@Tag(name = "Course Form", description = "Endpoints for handling course forms")
public class CourseFormController {
    private final CourseFormService courseFormService;

    @Operation(
            summary = "Upload a Course Form",
    ...
    public ResponseEntity<CourseFormResponse> uploadReceipt(@RequestPart("file") MultipartFile file,
        ...
        CourseFormResponse response = courseFormService.uploadReceipt(file, email, request.getStudentLevel());
    ...
    public ResponseEntity<List<CourseFormResponse>> getMyListings(@AuthenticationPrincipal UserInfoUserDetails principal) {
        if (principal == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        String email = principal.getUsername();
        return ResponseEntity.ok(courseFormService.getAllReceiptsByUser(email));
    }
    ...
    public ResponseEntity<List<CourseFormResponse>> getMyReceiptByLevel(@AuthenticationPrincipal UserInfoUserDetails principal, @PathVariable Level studentLevel) {
        if (principal == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        String email = principal.getUsername();
        return ResponseEntity.ok(courseFormService.getAllReceiptsByLevelByUser(studentLevel, email));
    }

    @Operation(
            summary = "Download approved documents as ZIP",
    ...
    public ResponseEntity<byte[]> downloadUserReceipt(@AuthenticationPrincipal UserInfoUserDetails principal)
        ...
        String email = principal.getUsername();
        FileDownloadResponse file = courseFormService.downloadReceiptByUser(email);
    ...
    public ResponseEntity<byte[]> downloadApprovedReceipts() throws IOException {
        byte[] zipBytes = courseFormService.downloadApprovedReceiptsAsZip();
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=approved-course-forms.zip")
