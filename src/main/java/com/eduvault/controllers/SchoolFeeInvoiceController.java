...
import com.eduvault.services.SchoolFeeInvoiceService;
...
@Tag(name = "School Fee Invoice", description = "Endpoints for handling School Fee Invoice")
public class SchoolFeeInvoiceController {
    private final SchoolFeeInvoiceService schoolFeeInvoiceService;


    @Operation(
            summary = "Upload a School Fee Invoice",
    ...
    public ResponseEntity<SchoolFeeInvoiceResponse> uploadReceipt(@RequestPart("file") MultipartFile file,
        ...
        SchoolFeeInvoiceResponse response = schoolFeeInvoiceService.uploadReceipt(file, email, request.getStudentLevel());
    ...
    public ResponseEntity<List<SchoolFeeInvoiceResponse>> getMyListings(@AuthenticationPrincipal UserInfoUserDetails principal) {
        if (principal == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        String email = principal.getUsername();
        return ResponseEntity.ok(schoolFeeInvoiceService.getAllReceiptsByUser(email));
    }
    ...
    public ResponseEntity<List<SchoolFeeInvoiceResponse>> getMyReceiptByLevel(@AuthenticationPrincipal UserInfoUserDetails principal, @PathVariable Level studentLevel) {
        if (principal == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        String email = principal.getUsername();
    ...
    public ResponseEntity<byte[]> downloadApprovedReceipts() throws IOException {
        byte[] zipBytes = schoolFeeInvoiceService.downloadApprovedReceiptsAsZip();
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=approved-sch-fee-invoice-receipts.zip")
