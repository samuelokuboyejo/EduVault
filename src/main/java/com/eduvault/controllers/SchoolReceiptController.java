...
import com.eduvault.services.SchoolFeeReceiptService;
...
@Tag(name = "School Fee", description = "Endpoints for handling School Fee receipts")
@RequestMapping("/sch-fee")
public class SchoolReceiptController {
    private final SchoolFeeReceiptService schoolFeeReceiptService;

    @Operation(
            summary = "Upload a School Fee Receipt",
    ...
    public ResponseEntity<SchoolFeeResponse> uploadReceipt(@RequestPart("file") MultipartFile file,
        ...
        SchoolFeeResponse response = schoolFeeReceiptService.uploadReceipt(file, email, request.getStudentLevel());
    ...
    public ResponseEntity<List<SchoolFeeResponse>> getMyListings(@AuthenticationPrincipal UserInfoUserDetails principal) {
        if (principal == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        String email = principal.getUsername();
        return ResponseEntity.ok(schoolFeeReceiptService.getAllReceiptsByUser(email));
    }
    ...
    public ResponseEntity<List<SchoolFeeResponse>> getMyReceiptByLevel(@AuthenticationPrincipal UserInfoUserDetails principal, @PathVariable Level studentLevel) {
        if (principal == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        String email = principal.getUsername();
        return ResponseEntity.ok(schoolFeeReceiptService.getAllReceiptsByLevelByUser(studentLevel, email));
    }
    ...
    public ResponseEntity<byte[]> downloadUserReceipt(@AuthenticationPrincipal UserInfoUserDetails principal)
        ...
        String email = principal.getUsername();
        FileDownloadResponse file = schoolFeeReceiptService.downloadReceiptByUser(email);
    ...
    public ResponseEntity<byte[]> downloadApprovedReceipts() throws IOException {
        byte[] zipBytes = schoolFeeReceiptService.downloadApprovedReceiptsAsZip();
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=approved-sch-fee-receipts.zip")
